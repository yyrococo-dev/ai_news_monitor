...
## Sprint 1 - 초기 (1주차)
- [ ] 1.1 스캐폴딩: 리포지토리, 가상환경, .gitignore, README 초안
- [ ] 1.2 Base interfaces: core/BaseCollector, core/BaseDeliverer (ABC)
- [ ] 1.3 storage: sqlite 초기화 스크립트 + storage/schema.py (sent_items, sources 테이블)
- [ ] 1.4 collectors: collectors/github_watcher.py (releases polling, tenacity 적용)
- [ ] 1.5 collectors: collectors/rss_collector.py (RSS 파서, feedparser 사용)
- [ ] 1.6 aggregator: URL 정규화, dedupe, PENDING 상태 로직
- [ ] 1.7 runner.py: 오케스트레이션(한 번 실행 후 종료형) + 로컬 실행 가이드
- [ ] 1.8 deliver: deliver/telegram_deliver.py (message truncation/pagination)
